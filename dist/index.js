...
var toc_inline_default = 'var u=new IntersectionObserver(o=>{for(let c of o){let n=c.target.id,d=document.querySelectorAll(`a[data-for="${n}"]`),e=c.rootBounds?.height;e&&d.length>0&&(c.boundingClientRect.y<e?d.forEach(t=>t.classList.add("in-view")):d.forEach(t=>t.classList.remove("in-view")))}});function i(o){o.classList.add("collapsed"),o.setAttribute("aria-expanded","false"),document.documentElement.classList.remove("toc-lock-scroll","toc-mobile-no-scroll")}function p(o){document.querySelectorAll(".explorer:not(.collapsed)").forEach(n=>{n.classList.add("collapsed"),n.setAttribute("aria-expanded","false"),document.documentElement.classList.remove("mobile-no-scroll","lock-scroll")}),o.classList.remove("collapsed"),o.setAttribute("aria-expanded","true"),document.documentElement.classList.add("toc-lock-scroll","toc-mobile-no-scroll")}function m(){document.querySelectorAll(".explorer-toggle").forEach(e=>{let t=()=>{document.querySelector(".explorer")?.classList.contains("collapsed")&&document.querySelectorAll(".toc").forEach(a=>i(a))};e.addEventListener("click",t),window.addCleanup?.(()=>e.removeEventListener("click",t))});let c=Array.from(document.querySelectorAll(".toc"));for(let e of c){let t=e.querySelector("button.toc-header.desktop-toc");if(t){let l=function(){this.classList.toggle("collapsed"),this.setAttribute("aria-expanded",this.getAttribute("aria-expanded")==="true"?"false":"true");let r=this.nextElementSibling;r&&r.classList.toggle("collapsed")};t.addEventListener("click",l),window.addCleanup?.(()=>t.removeEventListener("click",l))}let s=e.querySelector("button.mobile-toc");if(!s)continue;s.classList.remove("hide-until-loaded"),s.checkVisibility?.()&&i(e);let a=function(){e.classList.contains("collapsed")?p(e):i(e)};s.addEventListener("click",a),window.addCleanup?.(()=>{s.removeEventListener("click",a),i(e)}),e.querySelectorAll(".toc-content a").forEach(l=>{let r=()=>i(e);l.addEventListener("click",r),window.addCleanup?.(()=>l.removeEventListener("click",r))})}let n=()=>{document.querySelectorAll(".toc").forEach(e=>i(e))};document.addEventListener("prenav",n),window.addCleanup?.(()=>document.removeEventListener("prenav",n)),u.disconnect(),document.querySelectorAll("h1[id], h2[id], h3[id], h4[id], h5[id], h6[id]").forEach(e=>u.observe(e))}document.addEventListener("nav",m);document.addEventListener("render",m);\n';
